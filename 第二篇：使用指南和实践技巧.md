...
`/compact` 命令用来压缩对话上下文，当我们发现 Claude Code 的回答开始变傻变慢时就可以手动压缩一下对话上下文（对话上下文使用量超过 95% 的时候，Claude Code 会自动压缩）

#### 7、清空对话上下文

`/clear` 命令用来清空对话上下文。比如我们在同一个工作目录里完成了任务 A，然后要去做任务 B，但是任务 B 和任务 A 一点关系也没有，那就可以清空对话上下文后再开始任务 B。这样一来，我们就不用退出 Claude Code 后再重启，任务 B 的第一轮对话也能从一个空的对话上下文开始
