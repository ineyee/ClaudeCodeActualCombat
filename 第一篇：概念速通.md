...
* 切分就是指把这句话切分成一个一个的词汇小片段，**这一个一个的词汇小片段就是我们常说的 token，token 是大模型处理文本的基本单位，**注意一个 token 并不严格对应一个“常用词汇”，它有自己的切分规则（**这里共 7 个 token**）
...
③ → 这个 tokenId 串儿会被交给**大模型**处理，大模型内部基于我们提问的 tokenId 串儿经过一堆运算，会吐出一个它认为概率最高的 token 对应的 tokenId，注意大模型一次运算后只会吐出一个 tokenId
...
⑤ → 此时**大模型**并不会停下来，而是基于我们提问的 tokenId 串儿 + 新吐出的 tokenId，内部经过一堆运算，再吐出一个它认为概率最高的 token 对应的 tokenId
...
⑦ → 如此循环，直到大模型认为不需要再往下继续预测了，就会吐出一个特殊的 token “\<eos\>”字符串作为结束标识符，**我们**就得到了最终的答案（**共 6 个 token**）

![image-20260417173225688](img/image-20260417173225688.png)

但这次会话所消耗的 token 数可不止我们能看到的 **7 个输入 token + 6 个输出 token**，因为有太多输入和输出 token 我们看不到了，这个下面再说。
