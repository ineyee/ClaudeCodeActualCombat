...
但这次会话所消耗的 token 数可不止我们能看到的 **7 个输入 token + 6 个输出 token**，因为有太多输入和输出 token 是我们看不到的，下面会提到。**平均来讲，1 个 token ~= 0.75 个英语单词 ~= 1.5 个汉字，汉字更省 token，因为中文的信息密度更大，同样的语义下英文要更长。**
...
| 会话上下文                        | 我们提供啥代码片段或代码文件，它的会话上下文就是啥           | 根据我们的任务自行搜索相关文件和代码，自行构建上下文         |                                                              |
...
#### 3、提示词（每轮对话都传递什么，不止提示词，还有工具的执行结果等，提示词 + 工具的执行结果等就是会话上下文）
