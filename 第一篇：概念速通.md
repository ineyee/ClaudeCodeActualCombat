...
① → **我们**提问”浙江的省会是哪里？“（这里共 9 个字符）

② → 这句话会被交给 Tokenizer 处理、分两步：切分和映射

* 切分就是指把这句话切分成一个一个的词汇小片段，**这一个一个的词汇小片段就是我们常说的 token，token 是大模型处理文本的基本单位，**注意一个 token 并不严格对应一个“常用词汇”，它有自己的分词算法（**这里共 7 个 token**）
...
## 三、Tools --> 工具🛠️、函数💻（2024.05、2024.11）
...
|                                   | Claude App（Chat 模式）<br />轻量智能体                      | Claude Code<br />真正智能体                                  | Claude  App（Cowork 模式）<br />“真正智能体”                 |
| --------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
