...
**可见 MCP Server 的确就是一个工具提供者，而不是传统意义上的远程服务器，我们之所以会把 MCP Server 误认为是远程服务器，主要是因为绝大多数 MCP Server 里的工具都是在通过 API 访问某个远程服务器上的数据，就像我们编写的 weather-mcp-server 那样，只有少部分 MCP Server 是在访问本机计算机上的数据，iOS Simulator MCP Server 就是其中之一。所以我们常说的“给 Claude App 安装 Xxx MCP Server”其实就是给 Claude App 安装了一堆工具。**
